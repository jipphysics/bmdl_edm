...
cdef extern from "cpp_sbp_mod.h":
    cdef cppclass SBP_MOD:
        # Notice, the beta here corresponds to the meta_beta in the RMDL formulation.
        SBP_MOD( StaticGraph & I , int num_labels , double beta ) except +
        SBP_MOD( SBP_MOD & gm ) except +
        SBP_MOD & operator=( const SBP_MOD & other )
        #~SBP_MOD()
        ...
        #void set_random_b()
        #void update_theta()
        #double run_BP_step( StaticGraph::t_nn_iterator nn_it , double dumping , int verbose )
        #double run_BP_fast( int MC_steps , double err_tol , double dumping , double paramagnetic_tol , int verbose )
...
# === Python Wrapper for the C++ classes ===

#ifndef Included_cpp_sbp_rmdl
#define Included_cpp_sbp_rmdl
...
class SBP_RMDL {
    // This is the base-class for Graphical Models SBP_RMDL.
    // Here, the term "beta" actually represents "meta_beta" in our formulation. The simpler term "beta" is inherited from Zhang and Moore formulation of the BP-algorithm.
    ...
    std::vector< double >                                  _eta; // Local field eta[ s ]
    ...
    SBP_RMDL( const StaticGraph & I , int num_labels_ , double beta ) : _I( I ) , _num_labels( num_labels_ ) , _beta( beta ) {
        // The constructor initializes the SBP_RMDL graphical model in the paramagnetic phase.
    ...
    SBP_RMDL( SBP_RMDL & other ) : _I( other._I ) , 
    ...
    ~SBP_RMDL() {}
    SBP_RMDL & operator=( const SBP_RMDL & rhs ) {
        //_I            = rhs._I;
        assert( _I == other._I );
    ...
    double k_i( int i ) { return ( double ) _I.degree( i ); }
    double Kk_i( int i ) { 
        double _k_i = k_i( i );
        return _k_i / ( _K - _k_i );
    }
    ...
    void update_theta_and_eta() {
        for ( int s = 0 ; s < num_labels() ; s++ ) {
            ...
            _eta[ s ]   = 0.;
            for ( int i = 0 ; i < num_variables() ; i++ ) {
                _theta[ s ] += k_i( i )  * _b[ i ][ s ];
                _eta[ s ]   += Kk_i( i ) * _b[ i ][ s ];
            }
        }
    }
    ...
    double run_BP_step( StaticGraph::t_nn_iterator nn_it_ij , double dumping , int verbose ) {
        ...
        for ( int s_j = 0 ; s_j < num_labels() ; s_j++ ) {
            ...
            double suma = - .5 * beta() * ( _Kk_j * _theta[ s_j ] +  _k_j * _eta[ s_j ] );
        ...
        // Pre-update theta and eta
        for ( int s_i = 0 ; s_i < num_labels() ; s_i++ ) {
            _theta[ s_i ] -= _k_i  * _b[ i ][ s_i ];
            _eta[ s_i ]   -= _Kk_i * _b[ i ][ s_i ];
        }
        // Update b_i
        for ( int s_i = 0 ; s_i < num_labels() ; s_i++ ) {
            double suma = - .5 * beta() * ( _Kk_i * _theta[ s_i ] + _k_i * _eta[ s_i ] );
    ...
    t_BP_summary run_BP_fast( int MC_steps , double err_tol , double dumping , double paramagnetic_tol , int verbose ) {
        ...
        for ( BP_summary.run_MC_steps = 0 ; BP_summary.run_MC_steps < MC_steps ; BP_summary.run_MC_steps++ ) {
            ...
            for ( unsigned int l = 0 ; l < nn_it_pool.size() ; l++ ) {
                //max_err = std::max( max_err , run_BP_step( nn_it_pool[ l ] , dumping , verbose ) );
                BP_summary.max_err = std::max( BP_summary.max_err , run_BP_step( nn_it_pool[ l ] , annealing_dumping , verbose - 1 ) );
            }
            if ( verbose > 0 )
                std::cout << std::string( 10 - verbose , ' ' ) << "@run_BP_fast: MC_step=" << BP_summary.run_MC_steps << " max_err=" << BP_summary.max_err << " ann_dumping=" << annealing_dumping << std::endl;
            // Complete summary
    ...
    double H_i( int i , int s_i ) {
        return .5 * ( Kk_i( i ) * _theta[ s_i ] + k_i( i ) * _eta[ s_i ] );
    }
    ...
    friend std::ostream& operator<<( std::ostream& os , SBP_RMDL & gm ) {
       std::string sep = " ";
       os << "SBP_RMDL{" << std::endl;
...
std::ostream& operator<<( std::ostream& os , SBP_RMDL & gm );

#endif // Included_cpp_sbp_rmdl
